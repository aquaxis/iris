...
仕様3.4.2の変換は、幅を角括弧に入れて書く。
`x.sign_extend(32)`ではなく`x.sign_extend[32]()`である。
前者を出していた時期があり、それはIRISではないため
`iris2sv`が受け付けず、往復が閉じなかった。

| SystemVerilog | IRIS |
|---------------|------|
| `32'($signed(x))` | `x.sign_extend[32]()` |
| `8'(x)` （縮める） | `x.truncate[8]()` |
| `32'(x)` （広げる） | `x.extend[32]()` |
...
4. 変換結果の意味的等価性は、一般には保証しません（構文変換のみ）。
   ただし`example/`の6設計については、
   `iris2sv` → `sv2iris` → `iris2sv`の往復が同じ回路になることを
   `tools/formal/run.sh`が証明しています。
   **往復を字面で比べても答は出ません。**
   ALUの往復は冗長な`32'(32'(...))`だけ違い、
   diffはこれを差と呼び、等価性の証明は何も無いと言います
