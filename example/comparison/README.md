...
**ここで確かめているのはIRIS由来とVeryl由来の一致です。**
IRISの設計と`iris2sv`が出したSystemVerilogの一致は、
標本ではなく証明で確かめてあります。
...
`example/`の6設計すべてについて、すべての入力と到達しうるすべての状態で
一致することが、段数無しで証明されています。
仕組みは[`doc/formal_verification.md`](../../doc/formal_verification.md)にあります。

手書きのVeryl版との突き合わせは、証明の対象外です。
別々に書かれた2つの設計であり、比べられるのは振る舞いの標本だけです。
