...
テストベンチが通ることは、与えた入力について変換が正しかったという意味でしかない。
この設計については、**すべての入力について**正しいことが証明されている。
...
**2つのクロックがあっても証明できる。**
着手時は「非同期の境界をまたぐサイクル精度の等価性は定義されない」と考え、
同期化回路のところで設計を切り、クロック領域ごとに証明するつもりでいた。
それは要らなかった。
`equiv_make`は2つのクロックを他の入力と同じ自由入力として扱うので、
証明される主張は「両方のクロックのどんな波形に対しても、
2つのネットリストは同じに振る舞う」であり、これは定義されている。

**またぎが正しいという主張ではない。**
同期段数がそこで起こる準安定に足りているかは設計の問題であり、
変換が設計を保ったかとは別の問いである。
そちらはテストベンチの担当のままである。

この設計は、証明の過程で`iris2sv`の不具合を2件出した。
どちらも幅の式をコメントに置き換えたうえで変換成功と報告するもので、
`input logic [/* expr */-1:0] wr_data`のような出力になっていた。
原本では起きず、往復して幅が式になったときだけ通る経路だった。

仕組みは[`doc/formal_verification.md`](../../../doc/formal_verification.md)にある。
