...
IRISはIEEE 754の浮動小数点型を提供する。

| 型 | ビット幅 | 構成（符号／指数／仮数） |
|----|---------|-------------------------|
| `f32` | 32 | 1 / 8 / 23（IEEE 754単精度） |
| `f64` | 64 | 1 / 11 / 52（IEEE 754倍精度） |
...
- `iris-sim`のインタプリタは`f32`と`f64`の演算を実装している。
  値はIEEE 754のビット列として保持し、四則演算（`+ - * /`）と
  比較（`== != < <= > >=`、結果は1ビット）を native に評価する。
- 実数リテラル（`1.5`など）は単体では書式を持たない。書式は文脈から取る。
  - 被演算子から: `a + 1.5`の`1.5`は`a`の書式で符号化される。
  - 代入先から: `y = 1.5`の`1.5`は`y`の書式で符号化される。
  - 実数リテラルだけの式（`y = 1.5 + 2.25`）は、代入先の書式で畳み込む。
  - 浮動小数点の文脈が無い実数リテラルは、黙って0にせず評価時にエラーにする。
- 波形（VCD）は浮動小数点の信号を`$var real`として十進で出す（`r1.5`）。
  ビット列（`0x3FC00000`）ではない。
- コンパイル型バックエンド（`iris-compile`）もインタプリタと同じく`f32`/`f64`を
  評価する。`iris_runtime::ops::float_binop`を両者が共有するので結果は一致する。
  被演算子から書式を取る式（`a + 1.5`）も、代入先から取る式（`y = 1.5`、
  `y = 1.5 + 2.25`）も、両バックエンドで一致する（`tests/compiled.rs`で確認）。
- 浮動小数点の**メモリ**（`mem coef: f32[N]`）も両バックエンドで扱える。
  読み出しは要素の書式で印を付けるので、ビットではなく浮動小数点として使える。
- **SystemVerilogとの変換**（`iris sv`／`iris from-sv`）は`f32`をSVの`shortreal`、
  `f64`を`real`に対応させる。実数リテラルはそのまま渡す。往復（IRIS→SV→IRIS）で
  型と値が保たれることをconformance（`tools/conformance/fixtures/float.iris`）で確認する。
  なお`verilator`は`shortreal`を`real`に格上げする（警告のみ）ので、conformanceの
  float検体はverilator比較の対象群には入れていない。
- 形式的等価性の経路（`iris-formal`）は浮動小数点を**対象外**とし、明示して拒否する。
  等価性はyosysがビット単位で証明し、IEEE 754の実数はそこでビット分解できないため。
  これは道具の性質から来る意図した限界である（`doc/formal_verification.md`参照）。
- 機械学習向けの狭い形式（`f16`、`bf16`、`fp8`）とマイクロスケーリング形式は、
  `report_float.md`で採否を検討している。順次拡張する。
