...
**Verylはネイティブシミュレーターを持つ。**
`veryl-simulator`クレートがそれである。
Craneliftで素早く走り始め、GCCが最適化した実行ファイルを裏で作り、
できあがると走行中にそちらへ切り替わる。
テストベンチはVerylで書き、`veryl test`で走らせる。

**だからVerylの検証はSystemVerilogに落とすことに限られない。**
以前この資料は、Verylの実行手段を`veryl build`経由のVerilatorだけであるかのように書いていた。
それは正しくない。ネイティブシミュレーターがある。

Verylの公表値（veryl-lang.orgのブログ）では、
このネイティブシミュレーターはVerilatorより速いとされる。
初回はCraneliftの立ち上がりの速さで大きく、キャッシュ後もやや速い。
**これはVerylの公表値であって、この機械で測ったものではない。**

## 確かめていないこと

| | 内容 |
|---|---|
| `veryl`本体 | **この機械に入れられなかった。** wasmtimeがrustc 1.94を要求し、この機械は1.91.1。ネイティブシミュレーターも`veryl build`も、この機械では走らせていない |
| 形式的等価性 | この機械で`veryl`を動かせないため、Veryl自身の実行と突き合わせる経路をここでは通していない。**Verylの能力の問題ではなく、この環境の制約である** |
