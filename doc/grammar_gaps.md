...
**章と一致していることは、文法が意味を持つことを言わない。**

```
port_connection = identifier ":" expression ;
```

`expression`という規則はどこにも無い。他の規則はすべて`expr`を使う。
それでも3箇所（`tools/iris.ebnf`、`spec/04`、`spec/16`）が
同じ字面で書いていたので、突き合わせ検査は通っていた。

**同じ間違いを3箇所で共有していると、比べても見つからない。**

`grammar_check.py`に2つ足した。

| 検査 | 見るもの |
|---|---|
| 未定義の参照 | 使われているのに定義が無い規則名 |
| 到達不能 | `source_file`から辿り着けない規則 |

これで210規則、指摘0。`expression`は`expr`に直した。
**言語は変わっていない。**`expression`は何も指していなかった。
